...
_URL = "https://ftp.ncbi.nlm.nih.gov/pubmed/baseline/pubmed24n{file_id}.xml.gz"


def get_url(file_id):
    return _URL.format(file_id=str(file_id).rjust(4, "0"))
...
class PubmedConfig(datasets.BuilderConfig):
    ...
    def __init__(self, num_files, **kwargs):
        ...
        Args:
        num_files: int, the number of files to be downloaded,
...
class Pubmed(datasets.GeneratorBasedBuilder):
    ...
    def _split_generators(self, dl_manager: datasets.DownloadManager):
        ...
        urls = [get_url(i) for i in range(1, self.config.num_files + 1)]
